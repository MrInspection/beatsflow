...
import {Timer, Coffee, Trophy, GripVertical, BrainCircuit, Clock1, Clock3, Clock6} from "lucide-react"
...
export function WorkflowBlockItem({block, isSelected, isExecuting, isDragOverlay = false}: WorkflowBlockItemProps) {
          ...
          <ContextMenuTrigger asChild>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Marks the completion of your workflow</span>
              </div>
            </div>
          </ContextMenuTrigger>
          <ContextMenuContent className="rounded-2xl">
            <ContextMenuItem onClick={handleDelete} disabled={isExecuting} className="rounded-xl">
  ...
  return (
    <div ref={setNodeRef} style={isDragOverlay ? undefined : style} className="w-full">
      <ContextMenu>
        <ContextMenuTrigger asChild>
          <div className={cn(
          ...
          >
            <div className="flex items-center gap-1.5">
              {isDraggable && (
                <div className="cursor-grab p-1 touch-none h-full" {...attributes} {...listeners}>
              ...
              <section className="flex-1 flex items-center gap-2">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-medium">{getBlockTitle(block.type)}</h3>
                      <p className="text-xs text-muted-foreground line-clamp-1">{getBlockDescription(block.type)}</p>
                    </div>
        ...
        <ContextMenuContent className="rounded-2xl">
          <ContextMenuItem onClick={handleEdit} disabled={isExecuting} className="rounded-t-xl text-sm">
            <Edit3 className="size-4"/>
            Edit Block
          </ContextMenuItem>
          <ContextMenuSeparator/>
          <ContextMenuItem onClick={handleDelete} disabled={isExecuting} className="rounded-b-xl text-sm">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px] p-0 rounded-3xl">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Slider
                      ...
                      onValueChange={(values) => updateBlock(block.id, {duration: values[0]})}
                      className="flex-1"
                    />
              ...
              <>
                <div className="space-y-2">
                  <RadioGroup
                    ...
                    onValueChange={(value) => updateBlock(block.id, { duration: Number.parseInt(value) })}
                  ...
                  >
                    {[
                      ...
                      >
                        <RadioGroupItem value={String(option.value)} id={`r${option.value}`} className="sr-only" />
                        <div>
                          {option.icon === "Clock1" && <Clock1 className="size-8 text-primary" />}
                          {option.icon === "Clock3" && <Clock3 className="size-8 text-primary" />}
                          {option.icon === "Clock6" && <Clock6 className="size-8 text-primary" />}
                        </div>
                        <div>
                          <p className="font-medium text-lg">{option.value} minutes</p>
          ...
          <div className="flex justify-end border-t px-6 pb-6 pt-4">
            <Button className="w-full" onClick={() => setIsEditDialogOpen(false)}>Done</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
